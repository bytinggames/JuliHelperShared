...
namespace JuliHelper.Audio
{
    ...
    public class MyChannel32 : WaveStream, ISampleNotifier
    {
        ...
        public MyChannel32(WaveStream sourceStream, float volume, float pan)
        {
            PadWithZeroes = true;
        ...
        /// </summary>
        /// <param name="sourceStream">The source stream</param>
        public MyChannel32(WaveStream sourceStream)
            :
            ...
            lock (lockObject)
            {
                ...
                if (bytesWritten < numBytes)
                {
                    ...
                    while (this.sampleProvider.GetNextSample(out left, out right) && bytesWritten < numBytes)
                    {
                        ...
                        if (position + bytesWritten >= length)
                        {
                            position = 0;
                            sourceStream.Position = 0;
                        }
